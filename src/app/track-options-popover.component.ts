    ...
    <ion-content scrollY="false" class="ion-no-padding">
      <div class="local-glass-island compact-island">
        <ion-list lines="none" class="popover-list">
          ...
          >
            <ion-icon
              [name]="isCurrentlyVisible ? 'eye-off-outline' : 'eye-outline'"
              slot="start"
              [color]="isCurrentlyVisible ? 'danger' : 'primary'"
            ></ion-icon>
            <ion-label [color]="isCurrentlyVisible ? 'danger' : ''">
              <strong>{{
                isCurrentlyVisible
                  ? ('ARCHIVE.HIDE' | translate)
                  : ('ARCHIVE.SHOW' | translate)
              }}</strong>
            </ion-label>
          </ion-item>
          <ion-item button class="action-item" (click)="selectAction('edit')">
            ...
            ></ion-icon>
            <ion-label
              ><strong>{{ 'ARCHIVE.EDIT' | translate }}</strong></ion-label
            >
          </ion-item>
          <ion-item
            button
            class="action-item"
            (click)="selectAction('apply_dem')"
            [disabled]="trackItem.processingStatus === 'completed'"
          >
            <ion-icon
              name="earth-outline"
              slot="start"
            ...
            ></ion-icon>
            <ion-label
              ><strong>{{ 'ARCHIVE.APPLY_DEM' | translate }}</strong></ion-label
            >
          </ion-item>
          <ion-item button class="action-item" (click)="selectAction('move')">
            ...
            <ion-label
              ><strong>{{
                'ARCHIVE.MOVE_TO_FOLDER' | translate
              }}</strong></ion-label
            >
          </ion-item>
          ...
          <ion-item button class="action-item" (click)="selectAction('delete')">
            ...
            <ion-label color="danger"
              ><strong>{{ 'ARCHIVE.REMOVE' | translate }}</strong></ion-label
            >
          </ion-item>
        </ion-list>
      </div>
